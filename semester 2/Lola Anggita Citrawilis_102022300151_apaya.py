...
def insert_barang():
    ...
    if st.button('Tambah Barang'):
        conn = sqlite3.connect('gudang1.db')
        cursor = conn.cursor()
        cursor.execute("INSERT INTO barang_gudang (nama_barang, jumlah_barang, harga_barang) VALUES (?, ?, ?)",
...
def tampilkan_barang():
    ...
    if data:
        df = pd.DataFrame(data, columns=['ID Barang', 'Nama Barang', 'Jumlah Barang', 'Harga Barang'])
        st.write(df)
    else:
